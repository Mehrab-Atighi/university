...
annual_life_table = readxl::read_xlsx("F:/lessons/Master/ریاضیات بیمه زندگی/Excel learning/Life_Table.xlsx")
...
Total_net_premium = Total_Benefits /(12*Final_Premium(Age , N1 , m))
Total_Expense = Final_Expense(Total_net_premium  , S1)
Total_Expense_PV_0 = (0.3 * Total_Expense) + (0.175 * term_annuty(Age , 3 , 1))
monthly_Expense = Total_Expense_PV_0 / (12 * term_annuty(Age , 5 , 12))
...
d = cbind(Total_Benefits , Total_net_premium , Total_Expense ,
          Total_Expense_PV_0 ,monthly_Expense,
...
table = data.frame(year = rep(0:29 , each = 12) ,
                   ...
                   Expense = c(rep(monthly_Expense , 5*12) , rep(0 , 25*12)))
...
table$gross.P.without.father.death.coverage = table$net.P + table$Expense  + table$father.Death.coverage.P + table$father.Death.coverage.Expense
table = table[1:(m*N1),]
outputs = list(
  Calculations = d,
  Table = table
...
#  input = list(
#   DAD_Age = 10 , Age = 10 , N1 = 10 , N2 = 10 , S1 =1 , S2 = 1 , m = 12 , DAD_Coverage = "no"
#  )
